...
import { bulkLabel } from '@/lib/utils'
    ...
    <form onSubmit={handleSubmit} className="max-w-4xl mx-auto p-8">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select value={siteId} onChange={e => setSiteId(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400">
              <option value="">Select a site…</option>
              {sites.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Menu <span className="text-red-500">*</span></label>
            <select value={menuId} onChange={e => setMenuId(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400">
          ...
          <div>
            ...
            <input type="date" value={deliveryDate} onChange={e => setDeliveryDate(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Notes</label>
            <input value={notes} onChange={e => setNotes(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400" placeholder="Special instructions…" />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {totalItems > 0 && <span className="text-xs bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full">{totalItems} total units</span>}
        </div>
        {categories.map((cat: any) => (
          cat.menuItems?.length > 0 && (
            <div key={cat.id} className="mb-5">
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">{cat.name}</h3>
              <div className="space-y-2">
                {cat.menuItems.map((item: any) => {
                  const qty = itemMap[item.id] || 0
                  const bulkUnits = item.bulkQty > 1 ? Math.ceil(qty / item.bulkQty) : null
                  return (
                    <div key={item.id} className={`flex items-center gap-4 px-4 py-3 rounded-lg border transition-colors ${qty > 0 ? 'border-emerald-200 bg-emerald-50' : 'border-gray-100 bg-gray-50'}`}>
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-gray-800">{item.name}</span>
                          {item.bulkQty > 1 && <span className="text-xs text-gray-400 bg-white border border-gray-200 px-2 py-0.5 rounded-full">{item.unitLabel}</span>}
                        </div>
                        {item.description && <p className="text-xs text-gray-400 mt-0.5">{item.description}</p>}
                      </div>
                      <div className="flex items-center gap-2">
                        {qty > 0 && bulkUnits && (
                          <span className="text-xs text-emerald-600 whitespace-nowrap">→ {bulkUnits} {item.unitLabel}</span>
                        )}
                        <div className="flex items-center border border-gray-200 rounded-lg bg-white overflow-hidden">
                          <button type="button" onClick={() => setQty(item.id, String(Math.max(0, qty - 1)))}
                            className="px-3 py-1.5 text-gray-500 hover:bg-gray-100 text-sm">−</button>
      ...
      <div className="flex gap-3 justify-end">
        <button type="button" onClick={() => router.back()} className="px-5 py-2 border border-gray-200 text-gray-600 text-sm rounded-lg hover:bg-gray-50">
          Cancel
        </button>
        <button type="submit" disabled={saving} className="px-6 py-2 bg-emerald-600 text-white text-sm rounded-lg hover:bg-emerald-700 disabled:opacity-50 font-medium">
