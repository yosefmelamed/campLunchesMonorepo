...
`/totals` — select any date range and see every item's total quantity across ALL orders (menu orders + bulk pre-orders combined), broken down by source. Expandable rows show which sites/menus contributed each unit. Quick-range buttons for "this week", "next week", "this month".

### Bulk Pre-Orders
`/bulk-orders` — pre-ship specific items (e.g., OJ and cereal) at the start of the week, before individual menu orders. Bulk orders can target a specific site or all sites. When you create a menu order for a site/date that has an active bulk pre-order:
- Items with sufficient bulk coverage are highlighted in blue ("✓ fully covered by bulk")
- Partially covered items show remaining needed quantity in amber
- The `isBulkCovered` flag is saved on each order item for accounting

### Menu Templates
`/menus` — each menu now has a template of default items + quantities. When you create a new order and select a menu, the item quantities are automatically pre-filled from the template. You can then adjust as needed.

### Full CRUD
All entities now support full Create / Edit / Delete (or Archive):
- **Sites**: Archive/restore
- **Menus**: Archive with template item management (expand to preview)
- **Menu Items**: Archive per-item; delete categories (blocked if items exist)
- **Orders**: Edit + hard delete
- **Bulk Orders**: Full edit + delete
